...
public class ImixsIdAdapter extends EContentAdapter { // AdapterImpl
	...
	public void setTarget(Notifier newTarget) {
		if (ImixsBPMNPlugin.isImixsDataObject(newTarget) || ImixsBPMNPlugin.isImixsTask(newTarget)) {
		...
		if (imixsElement != null && type == Notification.ADD) {
			...
			if (notification.getNewValue() instanceof SequenceFlow) {
				...
				if (ImixsBPMNPlugin.isImixsTask(imixsElement)) {
					...
					if (seqFlow != null && ImixsBPMNPlugin.isImixsDataObject(seqFlow.getSourceRef())) {
...
//						}
				...
				if (ImixsBPMNPlugin.isImixsDataObject(imixsElement)) {
